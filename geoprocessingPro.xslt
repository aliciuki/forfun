...
<!-- GEOPROCESSING TOOL CONTENT -->
  ...
  <xsl:variable name="upper" select="'ABCDEFGHIJKLMNOPQRSTUVWXYZ'" />
	...
	<xsl:template name="gp">
		...
		<xsl:if test="(//tool/toolIllust[@src != '']) or (/metadata/Binary/Thumbnail/img[@src != ''])">
			<div>
				<h3><res:idIllus/></h3>
      ...
      <xsl:if test="/metadata/Binary/Enclosure[(@rel = 'side-panel-help')]">
        <div>
          <h3><res:idSidePanelIllus/></h3>
          <xsl:apply-templates select="/metadata/Binary/Enclosure" mode="gp" />
        </div>
      </xsl:if>
	...
	<xsl:template match="/metadata/Binary/Thumbnail[(img/@src != '')]" mode="gp">
      ...
      <img class="gp" id="illustration" title="{$label}" alt="{$label}">
        <xsl:attribute name="src"><xsl:value-of select="@src"/></xsl:attribute>
      </img>
    </xsl:for-each>
	</xsl:template>
	<xsl:template match="/metadata/Binary/Enclosure[(@rel = 'side-panel-help')]" mode="gp">
		<xsl:variable name="label" select="res:str('idSidePanelIllus')" />
		<xsl:for-each select="img">
      <img class="gp" id="sidePanelIllustration" title="{$label}" alt="{$label}">
        <xsl:attribute name="src"><xsl:value-of select="@src"/></xsl:attribute>
      </img>
    </xsl:for-each>
