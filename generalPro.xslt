...
<xsl:template name="arcgisElementNameExpander">
	<xsl:param name="ele" /><xsl:call-template name="elementText">
    <xsl:with-param name="ele" select="$ele" />
  </xsl:call-template>&#160;<xsl:if test="$ele/@Sync = 'TRUE'"><span class="sync">&#8660;</span>&#x2009;</xsl:if>
</xsl:template>

<xsl:template name="arcgisElementValueExpander">
	<xsl:param name="ele" />
	<xsl:param name="value" /><xsl:call-template name="elementText">
      <xsl:with-param name="ele" select="$ele" />
...
<xsl:template name="arcgisElementMany">
    ...
    <xsl:for-each select="$ele">
      <xsl:if test="(position() = 1)"><span class="ElementLabel">
          <xsl:call-template name="elementText">
            <xsl:with-param name="ele" select="$ele" />
          </xsl:call-template>
        </span>&#x2003;</xsl:if>
